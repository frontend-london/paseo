...
describe("ACP selection validity helpers", () => {
  ...
  test("classifies config-mirrored AntiGravity modes as config options", () => {
    const result = resolveACPModeSelection({
...
describe("ACPAgentSession Zed parity", () => {
  ...
  test("routes config-mirrored AntiGravity modes through session config during creation", async () => {
    const modeConfig = {
      id: "mode",
      name: "Mode",
  ...
  test("does not fall back to session mode for an invalid config-mirrored choice", async () => {
    ...
    const { internals, setSessionMode, setSessionConfigOption } = prepareConfiguredOverrideSession(
      session,
      {
        currentMode: "default",
        modeSource: "config",
        availableModes: [{ id: "bypassPermissions", label: "Skip Permissions" }],
        configOptions: [selectConfigOption("mode", ["default"], "default")],
      },
    );
...
describe("ACPAgentClient probe cleanup", () => {
  ...
  test("times out a hung catalog.resolve after session/new and terminates the probe", async () => {
    vi.useFakeTimers();
    const terminator = new FakeTerminator();
    const child = createProbeChildStub();
    Object.defineProperty(child, "pid", { value: 4244 });
    ...
    const client = new TestACPAgentClient({
      ...
      catalogStageTimeoutMs: 50,
      catalogProbeTimeoutMs: 200,
      catalogModelResolver: () => new Promise(() => undefined),
    });
    ...
    try {
      ...
      const expectation = expect(pending).rejects.toThrow(
        /ACP catalog\.resolve timed out after 50ms/,
      );
      await vi.advanceTimersByTimeAsync(50);
      await expectation;
      expect(terminator.terminated).toContain(child);
  ...
  test("does not leak late rejections after session/new timeout", async () => {
    vi.useFakeTimers();
    const terminator = new FakeTerminator();
    const child = createProbeChildStub();
    Object.defineProperty(child, "pid", { value: 4245 });
          ...
          connection: {
            ...
            newSession: () =>
              new Promise((_resolve, reject) => {
                setTimeout(() => reject(new Error("late session/new reject")), 100);
              }),
          } as unknown as ClientSideConnection,
          stderrChunks: [],
          spawnReady: Promise.resolve(),
